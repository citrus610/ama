    ...
    for (i32 i = 0; i < placements.get_size(); ++i) {
        ...
        if (chain.count > 0) {
            attacks.push_back(Attack {
                ...
                .count = chain.count,
                .frame = child.frame,
                .score_total = chain.score + child.score,
                .all_clear = child.field.is_empty(),
                .result = child.field,
                .eval = INT32_MIN
            });
        ...
        if (depth + 1 < queue.size()) {
            ...
            auto detect = Detect::detect_fast(child.field);

            if (detect.score.chain.count > 0) {
                attacks_detect.push_back(Attack {
                    .score = detect.score.chain.score,
                    .count = detect.score.chain.count,
                    .frame = child.frame + detect.score.needed,
                    .score_total = detect.score.chain.score + child.score,
                    .all_clear = false,
                    .result = detect.plan,
                    .eval = INT32_MIN
                });
